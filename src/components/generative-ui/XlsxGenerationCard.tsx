...
/**
 * XlsxGenerationCard — Generative UI card for the generate_xlsx / potomac_xlsx tools.
 * Features: progress ring, sheet tabs, tabular data preview, download XLSX.
 */
...
const BG_DARK  = 'rgba(33,115,70,0.15)';
const BG_LIGHT = 'rgba(33,115,70,0.08)';
...
export interface XlsxGenerationCardProps {
  toolCallId: string;
  toolName: string;
  input?: any;
  output?: any;
...
export function XlsxGenerationCard({
  toolCallId, input, output, externalOutput, state, errorText,
}: XlsxGenerationCardProps) {
  ...
  const title = input?.title || input?.topic || input?.subject || input?.prompt?.slice(0, 60)
    || outputData?.title || 'Excel Spreadsheet';

  // Sheet names from output: array of strings or derived from count
  const sheetNames: string[] = outputData?.sheet_names
    || (Array.isArray(outputData?.sheets) && typeof outputData.sheets[0] === 'string' ? outputData.sheets : null)
    || (outputData?.sheet_count ? Array.from({ length: outputData.sheet_count }, (_, i) => `Sheet${i + 1}`) : []);
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      ...
      const parsed = await parseFileForPreview(blob, outputData?.filename || `${title}.xlsx`);
      setParsedDoc(parsed);
    } catch {
  ...
  return (
    <div style={{ marginTop: 12, borderRadius: 16, overflow: 'hidden', border: `1px solid ${COLOR}28`, backgroundColor: cardBg, fontFamily: "'Instrument Sans', sans-serif", boxShadow: isDark ? `0 8px 32px ${COLOR}20` : `0 8px 32px ${COLOR}12`, animation: 'xlsxSlideIn 0.4s cubic-bezier(0.16,1,0.3,1)' }}>
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: mutedCol, marginTop: 3, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {sheetNames.length > 0 && <><span style={{ opacity: 0.35 }}>·</span><span>{sheetNames.length} sheet{sheetNames.length !== 1 ? 's' : ''}</span></>}
                <span style={{ opacity: 0.35 }}>·</span>
                <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 5px', borderRadius: 4, backgroundColor: `${COLOR}14`, color: COLOR, letterSpacing: 0.6, textTransform: 'uppercase', fontFamily: "'DM Mono', monospace" }}>XLSX</span>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 20, fontSize: 10.5, fontWeight: 700, letterSpacing: 0.5, fontFamily: "'DM Mono', monospace", flexShrink: 0, backgroundColor: statusPill.bg, color: statusPill.color, border: statusPill.border }}>
            {statusPill.label ?? (
              <><span style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: COLOR, animation: 'xlsxPulse 1.5s ease-in-out infinite', display: 'inline-block' }} /> GENERATING</>
            )}
          </div>
        </div>
          ...
          <div style={{ marginBottom: 12 }}>
                  ...
                  return (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: idx > currentPhase ? 0.38 : 1 }}>
                      {done
                        ...
                        : active
                          ? <div style={{ width: 16, height: 16, borderRadius: '50%', border: `2px solid ${COLOR}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><div style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: COLOR, animation: 'xlsxPulse 1.4s ease-in-out infinite' }} /></div>
            ...
            {progress > 5 && (
              <div style={{ fontSize: 10.5, color: mutedCol, textAlign: 'center', marginTop: 6, opacity: 0.65, fontFamily: "'DM Mono', monospace" }}>
                {progress < 50 ? '15–40 seconds remaining' : progress < 80 ? 'Almost there' : 'Wrapping up'}
              </div>
            )}
          </div>
        )}
        ...
        {isError && (
          <div style={{ padding: '11px 13px', borderRadius: 9, backgroundColor: isDark ? 'rgba(239,68,68,0.09)' : '#FEF2F2', border: '1px solid rgba(239,68,68,0.18)', color: isDark ? '#FCA5A5' : '#DC2626', fontSize: 12.5, marginBottom: 12 }}>
            ⚠ {errorText || 'Spreadsheet generation failed — please try again.'}
          </div>
        )}
              ...
              <div style={{ display: 'flex', gap: 3, marginBottom: 12, overflowX: 'auto', paddingBottom: 2 }}>
                {sheetNames.map((name, i) => (
                  <button key={i} onClick={() => setActiveSheet(i)} style={{
                    ...
                    border: `1px solid ${i === activeSheet ? COLOR : metaBdr}`,
                    borderBottom: i === activeSheet ? `2px solid ${COLOR}` : `1px solid ${metaBdr}`,
                    background: i === activeSheet ? `${COLOR}14` : metaBg,
                    color: i === activeSheet ? COLOR : mutedCol,
                    fontSize: 11, fontWeight: i === activeSheet ? 700 : 400,
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 16px', borderRadius: 11, marginBottom: 12, background: isDark ? `linear-gradient(135deg,${COLOR}1A 0%,${COLOR}0D 100%)` : `linear-gradient(135deg,${COLOR}0F 0%,${COLOR}06 100%)`, border: `1.5px solid ${COLOR}30` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: 1 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 9, background: isDark ? BG_DARK : BG_LIGHT, border: `1px solid ${COLOR}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: `0 2px 8px ${COLOR}20` }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" fill={COLOR} opacity="0.12" stroke={COLOR} strokeWidth="1.5" /><path d="M3 9h18M3 15h18M9 3v18M15 3v18" stroke={COLOR} strokeWidth="1.2" opacity="0.6" /></svg>
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: textColor, fontFamily: "'Syne', sans-serif", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{outputData.filename || `${title}.xlsx`}</div>
                    <div style={{ fontSize: 11, color: mutedCol, marginTop: 2, display: 'flex', gap: 6, fontFamily: "'DM Mono', monospace", flexWrap: 'wrap' }}>
                      {sheetNames.length > 0 && <span>{sheetNames.length} sheet{sheetNames.length !== 1 ? 's' : ''}</span>}
                      {formatSize() && <><span style={{ opacity: 0.35 }}>·</span><span>{formatSize()}</span></>}
                      {outputData.row_count && <><span style={{ opacity: 0.35 }}>·</span><span>{outputData.row_count.toLocaleString()} rows</span></>}
                      {outputData.formula_count && <><span style={{ opacity: 0.35 }}>·</span><span>{outputData.formula_count} formulas</span></>}
                    </div>
                  </div>
                </div>
              ...
              <div style={{ borderRadius: 10, overflow: 'hidden', border: `1px solid ${metaBdr}`, marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', backgroundColor: metaBg, borderBottom: previewOpen ? `1px solid ${metaBdr}` : 'none' }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: textColor, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" stroke={COLOR} strokeWidth="1.5" /><path d="M3 9h18M9 3v18" stroke={COLOR} strokeWidth="1.2" opacity="0.6" /></svg>
                    Data Preview
                  </span>
                  <button onClick={() => setPreviewOpen(!previewOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: mutedCol, fontSize: 10, fontFamily: "'DM Mono', monospace", transition: 'color 0.15s' }}
                ...
                {previewOpen && (
                  <div style={{ backgroundColor: isDark ? '#0D0D10' : '#FFFFFF' }}>
                    {previewLoading ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 160, gap: 8, color: mutedCol, fontSize: 12 }}>
                        <span style={{ width: 14, height: 14, border: `2px solid ${COLOR}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'xlsxSpin 0.8s linear infinite', display: 'inline-block' }} />
                        Loading data preview…
                      </div>
                    ) : parsedDoc?.type === 'table' && parsedDoc.tables ? (
                      <div style={{ overflowX: 'auto', maxHeight: 350, overflowY: 'auto', padding: '0 0 12px 0' }}>
                        {parsedDoc.tables[activeSheet] ? (
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5, fontFamily: "'Instrument Sans', sans-serif" }}>
                            <thead>
                              <tr>
                                {parsedDoc.tables[activeSheet].headers.map((h, hIdx) => (
                                  <th key={hIdx} style={{ padding: '6px 10px', textAlign: 'left', fontWeight: 700, fontSize: 10.5, textTransform: 'uppercase', letterSpacing: 0.5, color: COLOR, borderBottom: `1px solid ${metaBdr}`, backgroundColor: `${COLOR}09`, whiteSpace: 'nowrap' }}>
                                    {h}
                                  </th>
                                ))}
                              </tr>
                            ...
                            <tbody>
                              {parsedDoc.tables[activeSheet].rows.slice(0, 50).map((row, rIdx) => (
                                <tr key={rIdx} style={{ backgroundColor: rIdx % 2 === 0 ? 'transparent' : (isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.015)') }}>
                                  {row.map((cell, cIdx) => (
                                    <td key={cIdx} style={{ padding: '4px 10px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'}`, color: textColor, whiteSpace: 'nowrap', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                      {cell}
                                    </td>
                                  ))}
                                </tr>
                        ...
                        ) : <div style={{ padding: 20, color: mutedCol, fontSize: 12 }}>No data in this sheet</div>}
                        {(parsedDoc.tables[activeSheet]?.rows.length ?? 0) > 50 && (
                          <div style={{ fontSize: 10.5, color: mutedCol, padding: '6px 14px', fontStyle: 'italic' }}>
                            Showing first 50 of {parsedDoc.tables[activeSheet].rows.length} rows
                          </div>
                        )}
                      </div>
                    ) : (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 120, color: mutedCol, fontSize: 12 }}>
                        {parsedDoc?.content || 'No preview available'}
                      </div>
                    )}
                  </div>
                )}
      ...
      <style>{`
        @keyframes xlsxPulse   { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.45;transform:scale(0.82)} }
        @keyframes xlsxSpin    { 0%{transform:rotate(0deg)} 100%{transform:rotate(360deg)} }
        @keyframes xlsxSlideIn { 0%{opacity:0;transform:translateY(8px)} 100%{opacity:1;transform:translateY(0)} }
      `}</style>
    </div>
  );
}

export default XlsxGenerationCard;
